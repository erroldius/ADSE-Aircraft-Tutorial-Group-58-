...
import math
from ADSE1 import ShS1_stability
from ADSE1 import ShS1_control
from ADSE1 import ShS1_neutral
from ADSE1 import xcg_fwd1
from ADSE1 import xcg_aft1
from ADSE1 import ShS1
...
l_f = 36.57
l_n = -11.85 - 0.39 # distance from 1/4 MAC to end of Nacelle, should be negative in our case
b_n = (1.56 * 1.2) # nacelle width/diameter
...
CL_Ah_max = (W_approach/(0.5*1.225*(V_app**2)*S))*1.2
CL_landing = (W_approach/(0.5*1.225*(V_app**2)*S))*1.2
Clmax = CL_landing - (W_approach/(0.5*1.225*(Vt**2)*S))
print('Clmax LOOK HERE', Clmax)
xac_wlanding = 0.34
CM_flap = -0.45
ShS = Sh/S
...
###### Stability #########
gradient_stability = ((Cla_h_cruise / Cla_Ah_cruise) * (1 - de_da) * (l_h / MAC) * VhV ** 2)
intercept_stability = xac_Ah_cruise - SM

###### Control #########
gradient_control = (CLh_max / CL_Ah_max) * (l_h / MAC) * VhV ** 2
...
ShS_stability = xcg_range / gradient_stability - intercept_stability / gradient_stability
ShS_neutral = xcg_range / gradient_stability - xac_Ah_cruise / gradient_stability

ShS_control = xcg_range / gradient_control + intercept_control / gradient_control

###### Plotting #########

fig, ax = plt.subplots(figsize=(10, 7))

ax.plot(xcg_range, ShS_stability, 'b-', linewidth=2.0,
        label=f'Stability — cruise')
ax.plot(xcg_range, ShS_control, 'r-', linewidth=2.0,
        label='Controllability — approach')
ax.plot(xcg_range, ShS_neutral, 'b--', linewidth=1.5,
        label='Neutral stability (SM = 0)')

################ CG Range ##############
ax.axvline(xcg_fwd, color='green', linestyle=':', linewidth=1.2, label=f'CG fwd  = {xcg_fwd:.2f} MAC')
ax.axvline(xcg_aft, color='green', linestyle=':', linewidth=1.2, label=f'CG aft  = {xcg_aft:.2f} MAC')
ax.axvspan(xcg_fwd, xcg_aft, alpha=0.08, color='green', label='CG range')
ShS = np.round(ShS, 3)
ax.axhline(y=ShS, color='black', linestyle=':', linewidth=1.2, label=f'Current tail size = {ShS}')

############### Plot axes and labels ############

ax.set_xlabel(r'$x_{cg}\;/\;\overline{c}$  (fraction of MAC)', fontsize=13)
ax.set_ylabel(r'$S_h\;/\;S$', fontsize=13)
ax.set_title('Scissor Plot — Modified CRJ-EXX', fontsize=14, fontweight='bold')
...
ax.annotate('Stable\nregion', xy=(0.9, 0.32), fontsize=10, color='blue', alpha=0.5,
            ha='center', fontstyle='italic')
ax.annotate('',
            xy=(xcg_fwd, ShS),      # The start point in data coordinates
            xytext=(xcg_aft, ShS),  # The end point in data coordinates
...
bx.plot(xcg_range, ShS_stability, 'b-', linewidth=2.0,
        label=f'Stability — cruise')
bx.plot(xcg_range, ShS_control, 'r-', linewidth=2.0,
        label='Controllability — approach')
bx.plot(xcg_range, ShS_neutral, 'b--', linewidth=1.5,
        label='Neutral stability (SM = 0)')


bx.plot(xcg_range, ShS1_stability, 'y-', linewidth=2.0,
        label=f'Ref: Stability — cruise')
bx.plot(xcg_range, ShS1_control, 'g-', linewidth=2.0,
        label='Ref: Controllability — approach')
bx.plot(xcg_range, ShS1_neutral, 'y--', linewidth=1.5,
        label='Ref: Neutral stability (SM = 0)')



################ CG Range ##############
'''bx.axvline(xcg_fwd, color='green', linestyle=':', linewidth=1.2, label=f'CG fwd  = {xcg_fwd:.2f} MAC')
bx.axvline(xcg_aft, color='green', linestyle=':', linewidth=1.2, label=f'CG aft  = {xcg_aft:.2f} MAC')
bx.axvspan(xcg_fwd, xcg_aft, alpha=0.08, color='green', label='CG range')'''
...
bx.set_xlabel(r'$x_{cg}\;/\;\overline{c}$  (fraction of MAC)', fontsize=13)
bx.set_ylabel(r'$S_h\;/\;S$', fontsize=13)
bx.set_title('Scissor Plot - Both Aircraft', fontsize=14, fontweight='bold')
bx.legend(fontsize=9, loc='upper left')
bx.set_xlim(0, 1.0)
bx.set_ylim(0, 0.5)
bx.grid(True, alpha=0.3)

bx.annotate('Controllable\nregion', xy=(0.10, 0.31), fontsize=10, color='red', alpha=0.5,
            ha='center', fontstyle='italic')
bx.annotate('Stable\nregion', xy=(0.9, 0.32), fontsize=10, color='blue', alpha=0.5,
            ha='center', fontstyle='italic')
'''ax.annotate('',
